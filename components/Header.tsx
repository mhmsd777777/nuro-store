...
export default function Header(){
 const[open,setOpen]=useState(false),[cart,setCart]=useState(0),[fav,setFav]=useState(0);
 useEffect(()=>{const sync=()=>{try{const c=JSON.parse(localStorage.getItem("nuro-cart")||"[]");const f=JSON.parse(localStorage.getItem("nuro-favorites")||"[]");setCart(c.reduce((n:any,x:any)=>n+(x.qty||1),0));setFav(f.length)}catch{}};sync();window.addEventListener("storage",sync);return()=>window.removeEventListener("storage",sync)},[]);
